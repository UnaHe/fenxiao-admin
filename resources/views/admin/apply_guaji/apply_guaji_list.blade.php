...
<script type="text/template" id="tpl-withdraw-detail">
    @include("admin.apply_guaji.tpl_apply_guaji_detail")
</script>
...
$(function () {
    ...
    $(document).on("click", ".detail", function () {
        ...
        $.get("/apply_guaji/detail", {id: apply_id}, function (resp) {
            layer.close(loading);
            if(resp.code == 200){
                ...
                layer.open({
                    ...
                    yes: function(index, layero){
                        layer.confirm("确认挂机续期？",{icon:3}, function () {
                            var loading = layer.load(1, {
                                shade: [0.3,'#000']
                            });
                            $.post("/apply_guaji/confirm", {id: apply_id}, function (resp) {
                                if(resp.code == 200){
                                    layer.closeAll();
                    ...
                    btn2:function(index){
                        layer.confirm("确认拒绝用户申请？",{icon:3}, function () {
                            ...
                            $.post("/apply_guaji/refuse", {id: apply_id}, function (resp) {
                                if(resp.code == 200){
                                    layer.closeAll();
                                    layer.msg("操作成功",{icon:1});
                                    setTimeout(function () {

  ...
  <div class="box">
    <div class="box-body">
        <div class="form-group">
          <label for="title">全部主题图片设置<span style="color:#999">(建议尺寸 300x300像素)</span></label>
          <div>
            <img src="{{$all_tag_pic}}" class="pic" id="all_tag_pic" width="300" height="300" style="display: block;margin-bottom: 10px;"/>
            <div id="edit_all_tag_pic" class="btn btn-info">上传图片</div>       
          </div>
        </div>        
    </div><!-- /.box-body -->
  </div><!-- /.box -->  
...
<div class="modal edit-dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
			<label for="title">标签名称</label>
			<input type="text" class="form-control" id="name" placeholder="请输入标签名称">
        </div>

        <div class="form-group">
          <label for="title">图标样式</label>
          <input type="text" class="form-control" id="icon_class" >
        </div>

        <div class="form-group">
          <label for="title">图片<span style="color:#999">(建议尺寸 300x300像素)</span></label>
          <div>
            <img src="" class="pic" id="pic" width="300" height="300" style="display: block;margin-bottom: 10px;"/>
    ...
    <script type="text/javascript">
      ...
      $(function () {
        ...
        var table_param = {        
          "sAjaxSource": "{{route('admin.tag.list')}}",
          "iDisplayLength": 10,
          'columns':[
        ...
        $(document).on('click', '.edit', function(){
          ...
          dialog.find("#icon_class").val(data['icon_class']);
        ...
        $(document).on('click', '.delete', function(){
            ...
            var id = $(this).data("id");
            $.post('{{route("admin.tag.del")}}', {id: id}, function(res){
                    if (res.code == 200) {
        ...
        dialog.on('click', '.save', function(){
            ...
            icon_class: dialog.find("#icon_class").val(),
            pic: dialog.find("#pic").attr("src")
        	};
        	$btn.prop("disabled", true).html("保存中...");
            $.post('{{route("admin.tag.save")}}', data, function(res){
  ...
  window.upload_all_tag_pic = new plupload.Uploader({
      runtimes : 'html5,flash,silverlight,html4',
      browse_button : 'edit_all_tag_pic', // you can pass an id...
      ...
      init: {
          FilesAdded: function(up, files) {
              plupload.each(files, function(file) {
                  upload_all_tag_pic.start();
              });
          },
          UploadProgress: function(up, file) {
              $("#edit_all_tag_pic").html("上传中...("+file.percent+"%)");
          },
          ...
          FileUploaded: function(uploader,file,responseObject){
            ...
            if (data.state == "SUCCESS") {
              ...
              $.post("/admin/tag/allTagPic", {all_tag_pic: "/"+data.url}, function(resp){
                  if(resp.code == 200){
                        $.simplyToast('上传成功!', 'success');                    
                        $("#all_tag_pic").attr("src", "/"+data.url);              
                  }
              });
